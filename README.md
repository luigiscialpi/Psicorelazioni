...
- `formule?: FormulaCalcolo[]` — calcolo automatico di indici derivati da altri campi (es. un Totale), valutate in modo sicuro da `valutaFormule()` (nessuna eval arbitraria: `valutaFormulaSicura()` in `testTemplateEngine.ts`). In **Gestione Test** ogni riga può essere marcata "riga calcolata" con due modalità di autoring: **somma/media** (checkbox delle altre righe da combinare, l'espressione es. `({icv} + {irp}) / 2` è generata da `buildFormulaSemplice()`) o **avanzata** (espressione libera con autocompletamento delle chiavi, per pesature/sottrazioni che la modalità semplice non copre). Al caricamento di un template esistente, `parseFormulaSemplice()` prova a riconoscere se l'espressione salvata corrisponde esattamente a una somma/media semplice per ripopolare i checkbox; se la forma è più complessa ricade in automatico sulla modalità avanzata mostrando l'espressione così com'è.
  ...
  - **Limite noto**: rinominare l'etichetta/chiave di una riga già referenziata dalla formula di un'altra riga non aggiorna a cascata quel riferimento (nessun refactor automatico delle chiavi) — se capita, la formula a valle va sistemata a mano.
...
- **Gestione Test** usa una griglia unica (righe = `campiPrincipali`, colonne = `colonne`) al posto delle due liste separate di prima: cliccare un'intestazione di riga/colonna apre un pannello di configurazione sotto la griglia (`GrigliaTemplate`/`PannelloColonna`/`PannelloCampo` in `GestioneTest.tsx`). Nessun cambio nel formato dati sottostante: è solo un modo diverso di editare gli stessi `campiPrincipali`/`colonne` che il motore e l'export già conoscevano.
