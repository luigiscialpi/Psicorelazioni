...
docs/
  ...
  piano_ux_gestione_test.md  # Piano UX (diagnosi, principi, 6 fasi) seguito per rendere
                         # "Gestione Test" utilizzabile da chi non è programmatore — §11
...
|---|---|---|
| `pazienti` | Dati anagrafici REALI, isolati dal contenuto clinico | `nome`, `cognome`, `data_nascita`, `scuola_classe`, `codice` (riferimento interno libero, non univoco), `eta_approssimativa`, `sesso`, `tipo_consulto`, `owner_id` (UUID, isolamento per-utente) |
| `relazioni` | Relazioni importate o generate | `tipo` (`'importata'` \| `'generata'`), `tipo_relazione`, `paziente_id` (FK), `testo_markdown`, `testo_anonimizzato`, `testo_originale_path` (bucket `docx-originali`), `wizard_snapshot` (JSONB, **senza anagrafica**), `owner_id` (UUID) |
| `profilo_stile` | Record singolo per utente | `id` (UUID), `owner_id` (UNIQUE, UUID), `documento_stile` (Markdown), `num_relazioni_analizzate`, `template_rilevati` (JSONB), `updated_at` |
| `sessioni_wizard` | Bozze di wizard in corso (autosalvataggio) | `stato` (`'in_corso'` \| `'completata'` \| `'esportata'`), `risposte_wizard` (JSONB), `bozza_generata`, `relazione_finale_id` (FK), `owner_id` (UUID) |
| `professionista` | Record singolo per utente | `id` (UUID), `owner_id` (UNIQUE, UUID), `nome_completo`, `genere`, `titolo`, `specializzazione`, `partita_iva`, `codice_fiscale`, ... |
| `test_templates` | Definizioni dei test clinici (§6), inclusi i due built-in WISC-IV/NEPSY-II | `id` (testuale), `categoria`, `scala_default`, `campi_principali`/`gruppi_secondari` (JSONB), `nota_range`, `colonne`, `formule` (JSONB), `built_in`, `attivo`, `schema_version`, `owner_id` (UUID, nullable per built-in) |
...
- `campiPrincipali: CampoTest[]` — i punteggi principali (`key`, `label`, `descr?`, `scala?`, `evidenziato?`)
- `gruppiSecondari?: GruppoTest[]` — subtest raggruppati (es. CBCL: "Scale Sindromiche", "Scale DSM Oriented"). Ognuno genera una propria tabella nel DOCX (`makeSecondaryTestTable()`), oltre alla tabella principale — non compaiono mai *nella tabella principale*, ma hanno sempre la propria.
- `layoutTabelleSecondarie?: 'interleaved' | 'raggruppato'` — come vengono impaginate le tabelle dei gruppi secondari nel DOCX. Default `'interleaved'` (assente = comportamento storico): ogni tabella secondaria seguita subito dalla sua narrativa, una alla volta. `'raggruppato'`: tutte le tabelle del test (principale + tutte le secondarie) insieme, poi tutte le narrative insieme dopo. Configurato in Gestione Test insieme ai gruppi secondari; **nuova colonna SQL** (`layout_tabelle_secondarie` su `test_templates`, stesso discorso di `mostra_categoria_descrittiva`: richiede una migrazione reale).
...
- `colonne?: ColonnaTest[]` — una voce per ogni metrica da inserire per campo (es. `[{nome:'Punti T', scala:{tipo:'qi_wisc'}}, {nome:'Percentile'}]`), default `[{nome:'Punteggio'}]`. Ogni colonna ha un nome, una `scala` **opzionale e indipendente** da `scalaDefault`, un `mostraFasciaInTabella?: boolean` e un `evidenziato?: boolean`. Avere una `scala` calcola sempre la fascia (mostrata dal vivo nel wizard, sempre inviata a Gemini in `buildGeminiPayload`); comparire anche come colonna "Fascia `<nome>`" dedicata in tabella/DOCX è **opt-in esplicito** via `mostraFasciaInTabella` (default false: impostare un range non aggiunge colonne da solo). Retrocompatibile: le vecchie colonne salvate come semplice stringa vengono normalizzate al volo in `{nome: stringa}` da un `z.preprocess` in `ColonnaTestSchema`, nessuna migrazione dati necessaria per questi due campi (sono dentro il JSONB `colonne` già esistente).
- `mostraCategoriaDescrittiva?: boolean` — se `false`, la colonna "Categoria descrittiva" (fascia di riga, da `scalaDefault`/`campo.scala`, indipendente dalle "Fascia `<colonna>`" per colonna) non compare in tabella/DOCX. Default `true` (assente = comportamento storico invariato). A differenza degli altri campi di questa sezione, questo **è una nuova colonna SQL di primo livello** (`mostra_categoria_descrittiva` su `test_templates`), non annidata in un JSONB esistente: richiede una migrazione reale, non basta il redeploy del codice — `ALTER TABLE test_templates ADD COLUMN mostra_categoria_descrittiva boolean;` va lanciata a mano su Supabase.
- `evidenziato?: boolean` (su `CampoTest` e su `ColonnaTest`) — riga/colonna con shading neutro (`EVIDENZIATO_FILL` in `exportDocx.ts`, distinto dall'azzurro d'intestazione) nella tabella del DOCX esportato. Puro styling: irrilevante per Gemini, mai incluso in `buildGeminiPayload()`. Se sia la riga sia la colonna di una cella sono evidenziate, resta un solo shading (non additivo).
- `formule?: FormulaCalcolo[]` — calcolo automatico di indici derivati da altri campi (es. un Totale), valutate in modo sicuro da `valutaFormule()` (nessuna eval arbitraria: `valutaFormulaSicura()` in `testTemplateEngine.ts`). In **Gestione Test** ogni riga può essere marcata "riga calcolata" con due modalità di autoring: **somma/media** (checkbox delle altre righe da combinare, l'espressione es. `({icv} + {irp}) / 2` è generata da `buildFormulaSemplice()`) o **avanzata** (espressione libera con autocompletamento delle chiavi, per pesature/sottrazioni che la modalità semplice non copre). Al caricamento di un template esistente, `parseFormulaSemplice()` prova a riconoscere se l'espressione salvata corrisponde esattamente a una somma/media semplice per ripopolare i checkbox; se la forma è più complessa ricade in automatico sulla modalità avanzata mostrando l'espressione così com'è.
  - `RisultatoTest.formuleManuali?: Record<string, boolean>` — per singola relazione, le chiavi che l'utente ha sovrascritto a mano nel wizard: `valutaFormule()` non le ricalcola più (ma il valore resta disponibile come variabile per formule a valle, es. un totale generale sopra due sub-totali), finché non si torna al calcolo automatico da un tasto dedicato nel wizard.
  - **Rinominare un'etichetta non rompe più le formule**: in **Gestione Test**, se la chiave (`key`) di una riga/subtest è già referenziata dalla formula di un'altra riga (`righeCheReferenzianoChiave()` in `components/state/gestioneTestState.ts`), l'azione `UPDATE_CAMPO`/`UPDATE_CAMPO_GRUPPO` smette di rigenerarla dallo slug dell'etichetta ad ogni tasto digitato — l'etichetta resta libera di cambiare, la chiave no. Un cambio manuale della chiave (dentro "Impostazioni avanzate" in `PannelloCampo`) resta possibile ma mostra un avviso inline non bloccante se quella riga è referenziata altrove.
...
`RisultatoTest.commenti?: Record<string, string>` — commento libero per singolo campo/riga (chiave = `CampoTest.key`), pensato per note del professionista tipo "bambino stanco durante la prova". Va **sempre** a Gemini come contesto grezzo in `buildGeminiPayload()` (mai omesso), ma con un'istruzione diversa secondo `RisultatoTest.commentiVisibili?: Record<string, boolean>` (checkbox per singolo commento nel wizard, stesso pattern di `interpretabilita`: è un giudizio caso per caso sul singolo paziente, non una regola fissa nel template): se marcato visibile, viene anche stampato come nota di testo dopo la tabella (`generaCommentiVisibili()`) e Gemini viene istruito a non ripeterlo; se assente/false resta invisibile nel documento e Gemini lo usa solo per orientare l'interpretazione. In `exportDocx.ts` questa nota è ristampata in modo deterministico dalla stessa fonte dati in `flushDinamica()` (non parsata dal Markdown grezzo, che la contiene solo per l'anteprima a schermo): un paio di regex di quel parser (`isNotaRange`, e il pattern di skip iniziale di una sezione dinamica) sono abbastanza larghe da matchare per errore testo che comincia con `*Lettera`, quindi qualunque nuovo testo deterministico premesso alla narrativa va sempre anche escluso esplicitamente lì, non solo aggiunto alla generazione Markdown.
...
- `getColonneEffettive()` / `getValorePerColonna()` → punto unico di fallback e lettura per le colonne di un template, condiviso da `generaTabella()`, `buildGeminiPayload()` e da `makeTestTable()` in `exportDocx.ts`: evita che tabella a schermo, payload Gemini ed export Word divergano su nomi colonna o valori (`makeTestTable()` in passato ignorava del tutto `template.colonne` e stampava sempre una sola colonna "Punteggio" hardcoded, indipendentemente da come il template era configurato — corretto insieme all'introduzione del range per colonna)
- `calcolaNarrativaGruppi()` → descrizione testuale piatta dei subtest, per il payload Gemini
- `buildGeminiPayload()` → il testo che Gemini riceve per scrivere il commento narrativo. **Contiene solo dati grezzi (label/punteggio/fascia/commento), mai la tabella già renderizzata né la nota range col corsivo**: un LLM a cui si mostra contenuto già formattato come tabella o citazione, appena prima di chiedergli di commentarlo, tende a farne l'eco nella risposta. Tabella e nota range finale vengono inserite nel documento solo dopo, da `assemblaDocumentoMarkdown()` — Gemini non le vede mai renderizzate.
...
- `analizzaStile(corpus)` → un documento Markdown (il "Profilo di Stile", vedi `docs/profilo_di_stile.md` per un esempio reale) con: struttura standard delle relazioni, registro linguistico, formule ricorrenti da riprodurre, come vengono trattate le tabelle, terminologia preferita/da evitare, lunghezza/ritmo, e un'analisi per-test di tutti i test clinici rilevati nell'archivio (`## 7. Analisi dei Test Clinici Rilevati nell'Archivio`, seguita da un `### Test: Nome` per ciascuno). Per ogni test, oltre a "Struttura Colonne" (nomi esatti, nell'ordine in cui compaiono) il prompt chiede anche un **Esempio Valori** — una riga di dati reale e anonimizzata: serve a dare a `generaTemplateTest` (sotto) materiale concreto da cui dedurre quante colonne servono davvero, invece di indovinarle dalla sola descrizione testuale del test. Il marcatore esatto di questa sezione 7 è cercato per `indexOf` da `splitProfilo()` per gli aggiornamenti incrementali: va mantenuto invariato se si tocca il prompt.
...
- I dati sono isolati per utente tramite `owner_id` + RLS su tutte le tabelle; i `test_templates` built-in (WISC-IV/NEPSY-II) restano condivisi, i custom sono privati.
...
- Lo schema `responseSchema` di `generaTemplateTest` (§7) include un'unione discriminata (`scalaDefault`, tre forme diverse secondo `tipo`) e, dall'introduzione del range per colonna, richiede anche `colonne` come semplice array di nomi (mai range/scala, che restano sempre configurazione manuale in Gestione Test): la conversione Zod→JSON Schema e la validazione lato client sono testate (`core/testTemplate.test.ts`), ma quanto bene Gemini rispetti effettivamente quella forma va verificato con una chiamata reale, non è osservabile in sviluppo senza chiave API valida.
- (Risolto) `rilevaNomiTestDaProfilo`/`generaTemplateTest` chiamavano `callGeministructured` (typo, funzione inesistente): qualunque generazione reale via Gemini falliva con un `ReferenceError` a runtime, invisibile in modalità mock. In parallelo, `insertTestTemplate()` scriveva `t.campi_principali` invece di `t.campiPrincipali` (snake_case su un oggetto camelCase → `undefined`): siccome quella colonna è `NOT NULL`, qualunque nuovo template creato da zero falliva comunque il salvataggio, indipendentemente dal primo bug. Tenere a mente questo secondo pattern quando si aggiunge un campo a `TestTemplate`: il layer dati non ha un mapper automatico (vedi punto sopra), un refuso nel nome non viene segnalato a runtime finché non si guarda `tsc -b` esplicitamente (Vite/esbuild non fa type-check in dev/build).
...
- **Gestione Test** usa una griglia unica (righe = `campiPrincipali`, colonne = `colonne`) al posto delle due liste separate di prima: cliccare un'intestazione di riga/colonna apre un pannello di configurazione sotto la griglia (`GrigliaTemplate`/`PannelloColonna`/`PannelloCampo` in `GestioneTest.tsx`). Nessun cambio nel formato dati sottostante: è solo un modo diverso di editare gli stessi `campiPrincipali`/`colonne` che il motore e l'export già conoscevano. Il form segue una divulgazione progressiva (`docs/piano_ux_gestione_test.md`, tutte e 6 le fasi implementate): sempre visibili Nome/categoria, Scala, Tabella punteggi e Anteprima; collassati di default (ma aperti da soli se il template li usa già) i Gruppi secondari e le Opzioni aggiuntive; l'identificativo interno (`key`) non compare mai nel percorso principale, solo dentro un `<details>` "Impostazioni avanzate". Un nuovo template parte da un piccolo picker di preset (`PRESET_TEMPLATES` in `gestioneTestState.ts`) invece che da un form vuoto. Sotto i 700px la griglia con scroll orizzontale è sostituita da un elenco verticale (classi CSS `.griglia-template-desktop`/`.griglia-template-mobile` in `index.css`).
